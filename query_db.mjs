...
async function main() {
  const { data, error } = await supabase.from('membership_tiers').select('id, name, tier_key, discount_percentage');
  console.log(data);
  
  const { data: c, error: ce } = await supabase.from('store_customers').select('id, customer_id, store_id, tier').limit(5);
