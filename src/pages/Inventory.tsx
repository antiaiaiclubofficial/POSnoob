...
const Inventory = () => {
  const { 
    inventory, vendors, stockLogs, shopName, shopAddress, shopPhone, shopLogo, receiptHeader, transactions,
      ...
      <div className="flex-1 overflow-y-auto p-10 scrollbar-hide">
        ...
        {activeTab === 'master' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center"><div className="relative w-96"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={18}/><input className="w-full bg-white border border-gray-100 rounded-2xl pl-12 py-4" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}/></div><button onClick={() => { setEditingItem(null); setIsItemModalOpen(true); }} className="bg-[#1A1F3D] text-white px-8 py-4 rounded-2xl font-black">เพิ่มสินค้าใหม่</button></div>
            <div className="bg-white rounded-[40px] border border-gray-100 overflow-hidden"><table className="w-full"><thead><tr className="bg-gray-50/50"><th className="px-8 py-6 text-left">Product</th><th className="text-center">Stock</th><th className="text-center">Price</th><th>Vendor</th><th className="text-right">Action</th></tr></thead><tbody className="divide-y divide-gray-50">{filteredInventory.map(item => (
              <tr key={item.id} className="hover:bg-gray-50/50"><td className="px-8 py-6 font-black">{item.name}</td><td className="text-center">{item.stock} {item.unit}</td><td className="text-center">{currency}{item.price}</td><td>{item.isConsignment ? 'Consignment' : 'In-house'}</td><td className="px-8 py-6 text-right"><button onClick={() => { setEditingItem(item); setIsItemModalOpen(true); }} className="p-3 text-gray-300 hover:text-[#1A1F3D]"><Edit3 size={16}/></button></td></tr>
            ))}</tbody></table></div>
          </div>
        )}
