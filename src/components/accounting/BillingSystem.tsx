...
const BillingSystem = () => {
  ...
  // Auto-heal: Sync voided transactions with billing documents
  useEffect(() => {
    const { transactions } = useStore.getState();
    const autoGeneratedDocs = billingDocuments.filter(d => 
      d.status === 'Paid' && 
      (d.remarks === 'Auto-generated from POS' || d.referenceDocumentNo)
    );

    autoGeneratedDocs.forEach(doc => {
      const relatedTx = transactions.find(t => {
        if (doc.referenceDocumentNo) return t.id === doc.referenceDocumentNo;
        const txDateStr = t.createdAt ? format(new Date(t.createdAt), 'yyyy-MM-dd') : t.date;
        return (doc.date === txDateStr || doc.date === t.createdAt || (doc.date && doc.date.startsWith(txDateStr))) &&
               doc.totalAmount === t.amount;
      });

      if (relatedTx && relatedTx.status === 'voided') {
        updateBillingDocumentStatus(doc.id, 'Cancelled');
      }
    });
  }, [billingDocuments]);
      ...
      {previewDoc && previewDoc.type === 'short_receipt' ? (
        <ReceiptPreview 
          transaction={{
            ...
            details: { receiptNo: previewDoc.documentNo }
          }}
          shopName={shopName}
          shopLogo={shopLogo}
