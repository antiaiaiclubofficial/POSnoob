-- Migration: 20260906020000_skip_package_credit_in_auto_post_sales.sql
-- Description: Skip journal entry posting for sales transactions where payment_method is Package or Store Credit,
-- because their revenue and cash inflow were already recognized upon package purchase or credit top-up.
...
BEGIN
    -- Get sales transaction (cast id to text for safe comparison)
    SELECT * INTO v_tx FROM public.sales_transactions WHERE id::TEXT = p_sales_id;
    ...
    -- Skip journal entry creation for Package and Store Credit redemptions
    -- (Revenue and cash were already recognized when customer purchased the package or topped up credit)
    IF v_tx.payment_method IN ('Package', 'Store Credit') OR LOWER(COALESCE(v_tx.payment_method, '')) IN ('package', 'store credit') THEN
        RETURN QUERY SELECT true, NULL::TEXT, 'Skipped'::TEXT, 'Package/Store Credit redemption is not recognized as duplicate revenue'::TEXT;
        RETURN;
    END IF;
